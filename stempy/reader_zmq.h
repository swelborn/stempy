...
public:
   ...
   * and the generation is incremented. Thus, if we continue to use this barrier
   * in the code, there is no worry of a race condition.
   */
...
public:
  ...
  void process_frames(Functor& func, zmq::socket_t& pull_socket, int i)
  {
    ...
    m_generation_barrier.barrier(
      "Synchronized threads after processing incomplete frames.");
